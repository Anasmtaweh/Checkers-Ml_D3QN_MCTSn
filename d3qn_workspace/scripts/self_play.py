...
def main():
    ...
    trainer = D3QNTrainer(
        env, action_manager, encoder, model, optimizer, buffer, device
    )
    ...
    for episode in range(1, NUM_EPISODES + 1):
        ...
        while not done:
            ...
            encoded_state = encoder.encode(
                state, player=env.current_player, force_move_from=env.force_capture_from
            )
            ...
            if done:
                ...
                if winner == env.current_player:
                    ...
                    custom_reward = -0.5  # Punish Draws
            else:
                # Living Tax
            ...
            next_encoded = encoder.encode(
                next_state, player=env.current_player, force_move_from=next_force_from
            )
            next_mask = action_manager.make_legal_action_mask(
                env.get_legal_moves() if not done else []
            )

            buffer.push(
                encoded_state, action_id, custom_reward, next_encoded, done, next_mask
            )
        ...
        if episode % 10 == 0:
            print(
                f"Episode {episode} | Reward: {episode_reward:.3f} | Epsilon: {epsilon:.3f}"
            )

        if episode % 500 == 0:
            torch.save(
                model.online.state_dict(), f"{CHECKPOINT_DIR}/d3qn_{episode}.pth"
            )
