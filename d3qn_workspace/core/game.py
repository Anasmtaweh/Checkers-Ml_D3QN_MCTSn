"""
Checkers Environment Module.

Provides the main CheckersEnv class which wraps the board and rules
into an OpenAI Gym-like interface for Reinforcement Learning.
"""
...
class CheckersEnv:
    ...
    def _piece_and_king_counts_for(
    ...
    def _normalize_action(self, action: Any) -> Any:
        ...
        if (
            isinstance(action, (list, tuple))
            and len(action) == 1
            and self._is_capture_step(action[0])
        ):
    ...
    def _apply_capture_sequence(
        self, steps: List[CaptureStep], player: int
        ...
        for start, landing, jumped in steps:
            ...
            if abs(piece_before) == 1 and (
                (player == 1 and lr == 7) or (player == -1 and lr == 0)
            ):
                self.force_capture_from = None
    ...
    def step(self, action: Any):
        ...
        if self.done:
            ...
            return (
                ...
                0.0,
                True,
                {"winner": self.winner, "continue": False, "from": None},
            )
        ...
        if self._is_capture_step(normalized):
            ...
            last_pos, count, must_continue = self._apply_capture_sequence(
                [normalized], player
            )
    ...
    def legal_moves(self):
        if self.done:
            return []
        return CheckersRules.get_legal_moves(
            self.board.get_state(), self.current_player, self.force_capture_from
        )
