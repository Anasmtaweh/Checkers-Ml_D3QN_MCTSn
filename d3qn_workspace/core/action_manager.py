...
class ActionManager:
    ...
    def make_legal_action_mask(
        self, legal_moves: List[Any], player: int = 1
    ) -> torch.Tensor:
        """
    ...
    def _extract_start_landing(
        ...
        # FIX: Unwrap list if it contains a single step/move
        ...
        if isinstance(move, (tuple, list)) and len(move) == 2:
            if isinstance(move[0], (tuple, list)) and isinstance(
                move[1], (tuple, list)
            ):
    ...
    def flip_move(self, move: Move) -> Move:
        ...
        """
        start, end = move
        return ((7 - start[0], 7 - start[1]), (7 - end[0], 7 - end[1]))
