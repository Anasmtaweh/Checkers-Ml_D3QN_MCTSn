...
def parse_legal_moves(
    legal_moves: List[Any], action_manager
) -> Tuple[List[Move], Dict[int, Any]]:
    """
...
def normalize_move_format(move: Any) -> Any:
    ...
    if isinstance(move, list):
        ...
        if isinstance(move[0], (list, tuple)) and len(move[0]) == 3:
            return [
                tuple(tuple(p) if isinstance(p, list) else p for p in step)
                for step in move
            ]
    ...
    if isinstance(move, (tuple, list)):
        if len(move) == 2:
            # Simple move
            return (
                tuple(move[0]) if isinstance(move[0], list) else move[0],
