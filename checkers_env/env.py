...
class CheckersEnv:
    def __init__(self):
        ...
        self.done = False
        self.force_capture_from: Optional[Tuple[int, int]] = None  # position that must continue a chain
    ...
    def _check_game_over(self):
        ...
        moves = CheckersRules.get_legal_moves(self.board.get_state(), self.current_player)
        if len(moves) == 0:
    ...
    def _apply_capture_sequence(self, steps: List[CaptureStep], player: int):
        ...
        more_captures = CheckersRules.capture_sequences(self.board.get_state(), player, start_pos=last_pos)
        if more_captures:
    ...
    def step(self, action):
        ...
        # info can carry winner (int), continuation flags, and last position for forced captures
