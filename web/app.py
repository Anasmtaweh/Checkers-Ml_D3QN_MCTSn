...
def check_and_enforce_draw():
    ...
    if red_pieces == 1 and black_pieces == 1:
        ...
        if np.any(board == 2) and np.any(board == -2):
            print("🤝 DRAW DECLARED: Insufficient Material (1 King vs 1 King)")
            env.done = True
            env.winner = 0
            return
    ...
    if env.no_progress >= DRAW_LIMIT:
        print(f"🤝 DRAW DECLARED: 50-Move Rule ({env.no_progress} plies without progress)")
        env.done = True
        env.winner = 0
        return
...
def get_move():
    with game_lock:
        ...
        if model:
            # === ALPHAZERO LOGIC (IRON CURTAIN / PURE DEFENSE) ===
            if isinstance(model, AlphaZeroModel):
                mcts = mcts_module.MCTS(
                    model,
                    action_manager,
                    encoder,
                    # 1. Focus Deep (Narrow search, checking for traps)
                    c_puct=2.0,            
                    # 2. Grandmaster Depth
                    num_simulations=1600,
                    device=device,
                    
                    # --- THE INVINCIBLE SETTINGS ---
                    dirichlet_epsilon=0.0, # Zero Noise
                    draw_value=0.0,        # <--- CRITICAL. Accept the Draw.
                    search_draw_bias=-0  # No bias. Pure Math.
                )
