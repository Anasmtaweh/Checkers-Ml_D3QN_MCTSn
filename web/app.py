"""
Web Server for Checkers RL Agents.

This module provides a Flask application for playing Checkers against trained models.
"""
import os
import sys
import time
...
logger = logging.getLogger("CheckersWeb")
...
try:
    ...
    try:
        ...
        logger.warning("⚠️ D3QN Model class not found. DQN agents will fail to load.")
        D3QNModel = None
...
device = "cpu"
logger.info(f"🔥 Web App running on: {device}")

WEB_MCTS_SIMS = 1600
logger.info(f"⚡ MCTS Simulations set to: {WEB_MCTS_SIMS} (Full Strength)")
...
def load_agent(name, path):
    logger.info(f"Loading agent: {name} from {path}")
    try:
        checkpoint = torch.load(path, map_location=device)

        # Detect AlphaZero
        if isinstance(checkpoint, dict) and "model_state_dict" in checkpoint:
            logger.info("  -> Detected AlphaZero Checkpoint")
        ...
        # Fallback to DQN
        state_dict = (
            checkpoint.get("model_online", checkpoint)
        ...
        try:
            ...
            logger.info("  -> Fallback to LegacyDuelingDQN")
...
def check_and_enforce_draw():
    """Acts as the Tournament Director to force draws."""
    board = env.board.board

    # 1. MATERIAL CHECK (1 King vs 1 King)
    # Count pieces
    red_pieces = np.count_nonzero(board > 0)
    black_pieces = np.count_nonzero(board < 0)

    # If both sides have 1 piece, check if they are kings
    if red_pieces == 1 and black_pieces == 1:
        # Check if they are actually kings (value 2 or -2)
        if np.any(board == 2) and np.any(board == -2):
            logger.info("🤝 DRAW DECLARED: Insufficient Material (1 King vs 1 King)")
            env.done = True
            env.winner = 0
            return

    # 2. NO PROGRESS RULE (Standard is 40 or 50 moves. We use 50 moves = 100 plies)
    # env.no_progress counts 'plies' (half-moves) since last capture/promotion
    DRAW_LIMIT = 100

    if env.no_progress >= DRAW_LIMIT:
        logger.info(
            f"🤝 DRAW DECLARED: 50-Move Rule ({env.no_progress} plies without progress)"
        )
        env.done = True
        env.winner = 0
        return
...
def get_board_state():
    return {
        ...
        # --- NEW FIELD ---
        "draw_counter": int(env.no_progress),  # Send the count so we can show "35/100"
    }
...
def get_move():
    with game_lock:
        ...
        if env.done or not env.get_legal_moves():
            if env.done:
                logger.info(f"🏁 GAME OVER. Winner: {env.winner}")
            return jsonify(get_board_state())
        ...
        if model:
            # === ALPHAZERO LOGIC (SHARK MODE) ===
            if isinstance(model, AlphaZeroModel):
                mcts = mcts_module.MCTS(
                    model,
                    action_manager,
                    encoder,
                    # 1. Wide Search (See tricks)
                    c_puct=2.0,
                    # 2. Deep Search
                    num_simulations=1600,
                    device=device,
                    # --- CRITICAL SETTINGS ---
                    dirichlet_epsilon=0.0,  # No Noise (Skill)
                    draw_value=-0.5,  # Hate Draws (Aggression)
                    search_draw_bias=-0.05,
                    # --- ENABLE THE FIX ---
                    use_material_bias=True,  # <--- ACTIVATED
                )
                ...
                logger.info(
                    f"[AI {agent_name}] V={root.get_greedy_value():.3f} | N={root.visits} | T={duration:.2f}s"
                )

                best_action = int(np.argmax(probs))
                selected_move = mcts._get_move_from_action(
                    best_action, legal_moves, player=cp
                )
            ...
            # === DQN LOGIC ===
                ...
                mask = action_manager.make_legal_action_mask(legal_moves, player=cp).to(
                    device
                )
...
def human_move():
    with game_lock:
        ...
        logger.warning(f"⚠️ REJECTED MOVE: {start} -> {landing}")
        logger.warning(f"   Must be one of: {[ (m[0], m[1]) for m in legal_moves ]}")
