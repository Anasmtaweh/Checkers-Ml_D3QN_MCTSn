...
function renderBoard(state) {
    ...
    if (statusObj) {
        if (state.game_over) {
            if (state.winner === 0) statusObj.innerText = "🤝 GAME DRAWN";
            else statusObj.innerText = "🏁 GAME OVER";
        } else {
            // Show turn AND Draw Counter
            // 100 plies = 50 moves. 
            const movesLeft = Math.ceil((100 - state.draw_counter) / 2);
            let status = (state.current_player === 1 ? "🔴 Red's Turn" : "⚫ Black's Turn");
            
            // Only show counter if it's getting close (e.g., last 20 moves)
            if (state.draw_counter > 20) {
                status += ` | Draw in ${movesLeft} moves`;
            }
    ...
    if (winnerObj) {
        if (state.game_over) {
            if (state.winner === 1) winnerObj.innerText = "🏆 RED WINS!";
            else if (state.winner === -1) winnerObj.innerText = "🏆 BLACK WINS!";
            else winnerObj.innerText = "🤝 DRAW (50-Move Rule)"; // Explicit text
