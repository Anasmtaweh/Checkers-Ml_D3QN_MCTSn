...
class HybridMCTS(MCTS):
    ...
    def _expand_node(self, node: AlphaNode, env: CheckersEnv) -> float:
        ...
        if player == -1: material_value = -material_value
...
def run_match():
    ...
    mcts_standard = MCTS(
        model=model, action_manager=manager, encoder=encoder, 
        num_simulations=SIMULATIONS, c_puct=1.0, device=DEVICE, dirichlet_epsilon=0.0
    )
    ...
    while not env.done:
        ...
        move = active_mcts._get_move_from_action(action_id, env.get_legal_moves(), env.current_player)
        ...
        print(f"Move {env.move_count}: {name} moves. (Eval: {root.get_greedy_value():.2f})")
        env.step(move)
