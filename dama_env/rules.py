...
class Rules:
        ...
        for r in range(8):
            for c in range(8):
                if board[r, c] == player or board[r, c] == 2 * player:
                    for dr in Rules._piece_directions(board, r, c):
                        for dc in (-1, 1):
                            nr, nc = r + dr, c + dc
    ...
    def _capture_sequences_from(board, r, c, player):
        ...
        for dr in Rules._piece_directions(board, r, c):
            for dc in (-1, 1):
                ...
                if Rules._is_opponent(board[mid_r, mid_c], player) and board[land_r, land_c] == 0:
                    # simulate jump
                    ...
                    subsequent = Rules._capture_sequences_from(new_board, land_r, land_c, player)
    ...
    def capture_sequences(board, player, start_pos=None):
        ...
        for r, c in positions:
            captures.extend(Rules._capture_sequences_from(board, r, c, player))
        return captures
    ...
    def get_legal_moves(board, player, forced_from=None):
        ...
        """
        captures = Rules.capture_sequences(board, player, forced_from)
        if captures:
            return captures
        return Rules.simple_moves(board, player)
