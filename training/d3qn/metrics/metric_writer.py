...
class D3QNMetricWriter:
    def __init__(self, base_dir: str = "logs/d3qn", run_metadata: Optional[dict] = None):
        ...
        # Winrate CSV (more detailed fields)
        ...
        if write_header:
            self.winrate_writer.writerow(
                ["episode", "win_rate", "first_win_rate", "second_win_rate", "draw_rate", "lr"]
            )
    ...
    def log_winrate(self, episode: int, ddqn_wins: int, random_wins: int, draws: int, win_rate: float) -> None:
        total = ddqn_wins + random_wins + draws
        draw_rate = draws / total if total else 0.0
        self.winrate_writer.writerow([episode, win_rate, None, None, draw_rate, None])
    ...
    def write_eval(
        ...
        """Log evaluation metrics with split win rates."""
        self.winrate_writer.writerow([episode, win_rate, first_win_rate, second_win_rate, draws, lr])
        self.winrate_file.flush()
