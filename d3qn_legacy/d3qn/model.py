...
class DuelingDQN(nn.Module):
    """
    Dueling Deep Q-Network architecture for Checkers.
    ...
    This separation helps the network learn which states are valuable
    independently from which actions are best in each state.
    """
    ...
    def __init__(self, action_dim: int, device: Union[str, torch.device] = "cpu"):
        ...
        self.value_fc2 = nn.Linear(512, 1)  # Single scalar value

        # ================================================================
        # Advantage Stream: A(s, a)
        # ================================================================
        # Estimates the advantage of each action in state s
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Value Stream: V(s)
        value = F.relu(self.value_fc1(x))  # (batch, 512)
        value = self.value_fc2(value)      # (batch, 1)

        # Advantage Stream: A(s, a)
        advantage = F.relu(self.advantage_fc1(x))  # (batch, 512)
        advantage = self.advantage_fc2(advantage)  # (batch, action_dim)
...
class D3QNModel:
    ...
    def get_q_values(self, state: torch.Tensor, use_target: bool = False) -> torch.Tensor:
        """
        ...
        return network.get_q_values(state)
...
if __name__ == "__main__":
    print("="*70)
    print("D3QN MODEL ARCHITECTURE TEST")
    print("="*70)

    # Test configuration
    action_dim = 168  # Typical for checkers
    device = "cpu"
    ...
    q_values = model.get_q_values(single_state)
    ...
    batch_q_values = model.get_q_values(batch_state)

    print(f"Input shape: {batch_state.shape}")
    print(f"Output shape: {batch_q_values.shape}")
    print(f"Output range: [{batch_q_values.min().item():.4f}, {batch_q_values.max().item():.4f}]")
    ...
    target_q_values = model.get_q_values(single_state, use_target=True)

    print(f"Target Q-values shape: {target_q_values.shape}")
    print(f"Online and Target match: {torch.allclose(q_values, target_q_values)}")

    # Test weight update
    print("\n" + "-"*70)
    print("Test 4: Network Updates")
    print("-"*70)
    ...
    q_values_modified = model.get_q_values(single_state)
    target_q_values_old = model.get_q_values(single_state, use_target=True)

    print(f"Online modified: {not torch.allclose(q_values, q_values_modified)}")
    print(f"Target unchanged: {torch.allclose(target_q_values, target_q_values_old)}")

    # Update target network
    model.update_target_network()
    target_q_values_new = model.get_q_values(single_state, use_target=True)

    print(f"Target updated: {torch.allclose(q_values_modified, target_q_values_new)}")
