...
sys.path.append(
    os.path.abspath(os.path.join(os.path.dirname(__file__), "../../d3qn_workspace"))
)
        ...
        with torch.no_grad():
            if epsilon > 0 and random.random() < epsilon:
                ...
                state = self.encoder.encode(
                    board, player=player, force_move_from=info.get("force_capture_from")
                )
                if state.dim() == 3:
...
def play_match(
    env, az_agent, d3qn_agent, action_manager, az_player_color, num_games=10
):
    az_wins = 0
    d3qn_wins = 0
    draws = 0
    ...
    for _ in range(num_games):
        ...
        while not done:
            ...
            if player == az_player_color:
                ...
                action_probs, _ = mcts.get_action_prob(
                    env, temp=MCTS_TEMP, training=False
                )
                ...
                move, _ = d3qn_agent.select_action(
                    env.board.board,
                    player,
                    legal_moves,
                    epsilon=D3QN_EPSILON,
                    info=info,
                )
...
def evaluate_checkpoints(checkpoints_dir, d3qn_path, output_dir, games_per_side=10):
    ...
    for ckpt_path in tqdm(checkpoints, desc="Evaluating Checkpoints"):
        ...
        az_wins_r, d3qn_wins_b, draws_r = play_match(
            env,
            az_model,
            d3qn_agent,
            action_manager,
        ...
        az_wins_b, d3qn_wins_r, draws_b = play_match(
            env,
            az_model,
            d3qn_agent,
            action_manager,
    ...
    if not df.empty:
        ...
        plt.title(
            f"AlphaZero vs D3QN (Sims={MCTS_SIMULATIONS}, MaxMoves={MAX_MOVES_PER_GAME})"
        )
