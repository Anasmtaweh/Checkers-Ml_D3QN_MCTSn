...
MODEL_PATH = os.path.join(PROJECT_ROOT, "mcts_workspace/checkpoints/alphazero/checkpoint_iter_239.pth")
AZ_SIMS = 1600
    ...
    def _evaluate(self, board):
        ...
        for r in range(8):
            for c in range(8):
                ...
                if 2 <= r <= 5 and 2 <= c <= 5: base_val += 0.2
                
                # 2. Back Row Defense (Hard to get Kinged)
                if piece == 1 and r == 0: base_val += 0.5  # P1 home row
                if piece == -1 and r == 7: base_val += 0.5 # P2 home row
                
                if piece > 0: score += base_val
                else: score -= base_val
        return score
...
def load_az():
    ...
    model.network.load_state_dict(torch.load(MODEL_PATH, map_location=DEVICE)[ "model_state_dict"])
    model.eval()
...
def run_gauntlet():
    ...
    for enemy in opponents:
        ...
        for i in range(2):
            ...
            mcts = MCTS(model, manager, encoder, num_simulations=AZ_SIMS, c_puct=1.0, device=DEVICE)
            
            while not env.done:
                is_az_turn = (env.current_player == 1 and az_p1) or (env.current_player == -1 and not az_p1)
