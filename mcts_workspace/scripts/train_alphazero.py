...
CSV_LOG_FILE = os.path.join(
    SCRIPT_DIR, "..", "data", "training_logs", "alphazero_training.csv"
)
...
def main():
    ...
    if args.resume is not None:
        ...
        try:
            ...
            if os.path.exists(BUFFER_PATH):
                ...
                print(
                    "⚠️  Warning: Replay buffer not found. Starting with empty buffer."
                )
    ...
    for iteration in range(start_iter + 1, CFG["NUM_ITERATIONS"] + 1):
        ...
        if "phases" in CFG:
            ...
            mcts.dirichlet_alpha = CFG.get(
                "DIRICHLET_ALPHA", 0.3
            )  # <--- ADDED: Critical for the 1.0 Hack
        ...
        print(
            f"Sims: {mcts.num_simulations} | Bias: {mcts.search_draw_bias} | Alpha: {mcts.dirichlet_alpha}"
        )

        # Run Self Play
        sp_stats = trainer.self_play(
            CFG["GAMES_PER_ITERATION"], verbose=True, iteration=iteration
        )
        ...
        # Print Summary
        print(
            f"  [Stats] P1: {log_data['p1_win_rate']:.1%} | P2: {log_data['p2_win_rate']:.1%} | Draw: {log_data['draw_rate']:.1%}"
        )
