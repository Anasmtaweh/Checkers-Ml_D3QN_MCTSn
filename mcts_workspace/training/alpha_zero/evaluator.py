    ...
    def play_match(self, model_p1, model_p2, num_games=10, mcts_sims=300):
        ...
        for i in range(num_games):
            ...
            if i % 2 == 0:
                ...
                if winner == 1:
                    results["l"] += 1  # P2 won as Red
                elif winner == -1:
                    results["w"] += 1  # P1 won as Black
                else:
                    results["d"] += 1
    ...
    def _play_single_game(self, red_mcts, black_mcts):
        ...
        while not done:
            ...
            if env.current_player == 1:
                ...
                move = red_mcts._get_move_from_action(
                    action_id, legal_moves, player=env.current_player
                )

            else:
                action_probs, _ = black_mcts.get_action_prob(
                    env, temp=0, training=False
                )
                action_id = int(np.argmax(action_probs))
                move = black_mcts._get_move_from_action(
                    action_id, legal_moves, player=env.current_player
                )

            if move is None:
                print(
                    f"⚠️ Illegal/Null move by Player {env.current_player} at move {moves}. Forcing Loss."
                )
...
def monitor_checkpoints(
    ...
    if not os.path.exists(best_model_path):
        ...
        while True:
            ...
            if list_of_files:
                ...
                copyfile(
                    latest_file, os.path.join(hof_dir, f"champion_iter_{iter_num}.pth")
                )
                break
            else:
    ...
    while True:
        ...
        for challenger_path in new_files:
            ...
            w1, l1, d1 = arena.play_match(
                challenger, current_champion, num_games=10, mcts_sims=300
            )
            ...
            if len(hof_files) > 0:
                ...
                for ghost_path in ghosts:
                    ...
                    gw, gl, gd = arena.play_match(
                        challenger, ghost, num_games=games_per_ghost, mcts_sims=300
                    )
                    w2 += gw
                    l2 += gl
                    d2 += gd
            ...
            if win_rate >= 0.55:
                ...
                copyfile(
                    challenger_path,
                    os.path.join(hof_dir, f"champion_iter_{iter_num}.pth"),
                )
                current_champion = load_model(best_model_path, arena.device)
            else:
