...
class MCTS:
    ...
    def __init__(
        ...
        search_draw_bias: float = -0.02,
        skip_root_sims_on_forced: bool = True,
        use_material_bias: bool = False,
    ):
        self.model = model
        ...
        self.use_material_bias = use_material_bias
        self.model.eval()

    def get_action_prob(
        self, env, temp: float = 1.0, training: bool = True
    ) -> Tuple[np.ndarray, AlphaNode]:
        ...
        self.is_endgame = total_pieces <= 16 or min(red_count, black_count) <= 6
        ...
        if self.skip_root_sims_on_forced and len(legal_moves) == 1:
            ...
            if env.current_player == -1:
                sl = self.action_manager.flip_move(sl)
            ...
            root.children[action_id] = AlphaNode(
                prior=1.0, parent=root, action_taken=action_id
            )
            return probs, root
    ...
    def _search(self, node: AlphaNode, env: CheckersEnv) -> float:
        ...
        if best_child is None:
            raise RuntimeError(
                "MCTS selection failed: Node has children but no child selected."
            )

        player_before = env.current_player
        move = self._get_move_from_action(
            best_action, env.get_legal_moves(), env.current_player
        )
    ...
    def _evaluate_aggressive(self, board, player):
        """Calculates Material + Positional Advantage."""
        score = 0
        for r in range(8):
            for c in range(8):
                piece = board[r][c]
                ...
                # 1. Base Material
                val = 5.0 if abs(piece) == 2 else 3.0  # Kings worth more now

                # 2. Positional Bonus (Control the Center)
                # Rows 3,4 and Cols 2,3,4,5 are the "Kill Zone"
                if 3 <= r <= 4 and 2 <= c <= 5:
                    val += 0.5

                # 3. Advancement Bonus (Push Men forward)
                if piece == 1:  # Red Man
                    val += (7 - r) * 0.1  # Closer to King = Better
        ...
        # Return relative score for the current player
        final_score = score if player == 1 else -score
        return np.tanh(final_score / 5.0)

    def _expand_node(self, node: AlphaNode, env: CheckersEnv) -> float:
        board = env.board.get_state()
        player = env.current_player

        # 1. Neural Network Prediction (The Brain)
        ...
        node_value = float(value)

        # 2. THE WARLORD INJECTION (Inference Only)
        if self.use_material_bias:
            strat_score = self._evaluate_aggressive(board, player)

            # BLEND: 60% Brain, 40% Aggression
            # We INCREASED the aggression influence to override the shuffling.
            node_value = (0.6 * node_value) + (0.4 * strat_score)
    ...
    def _select_child(self, node: AlphaNode) -> Tuple[int, Optional[AlphaNode]]:
        ...
        # --- THE SAFETY OVERRIDE ---
        # 1. Standard PUCT
        current_cpuct = self.c_puct

        # 2. ONLY increase PUCT if we are actively training with Dirichlet Noise enabled.
        # This is a hacky but 100% reliable way to detect "Training Mode".
        # If epsilon > 0, we are training -> Allow exploration.
        # If epsilon == 0 (Web App), we are inferencing -> BLOCK exploration.
        if getattr(self, "is_endgame", False) and self.dirichlet_epsilon > 0:
            current_cpuct *= 2.0
    ...
    def _get_move_from_action(
        self, action_id: int, legal_moves: List, player: int = 1
    ) -> Optional[Any]:
        mp = self.action_manager.get_move_from_id(action_id)
        if mp == ((-1, -1), (-1, -1)):
            return None
        if player == -1:
            mp = self.action_manager.flip_move(mp)
        for move in legal_moves:
            if self.action_manager._extract_start_landing(move) == mp:
                return move
        return None
    ...
    def _copy_env(self, env: CheckersEnv) -> CheckersEnv:
        ...
        new_env = CheckersEnv(
            max_moves=env.max_moves, no_progress_limit=env.no_progress_limit
        )
